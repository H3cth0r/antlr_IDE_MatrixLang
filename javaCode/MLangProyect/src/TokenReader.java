...
/*    public static void main (String [] args) throws IOException{
        ...
        // [line, char_pos, color, token]
        // If detected some reserved word or a number, add to toks array.
        String[][] toks = new String[the_tokens.size()][];              // Array to return to the IDE for higlighting
        String token_name_iterator = "";
        for(Integer i = 1; i < the_tokens.size(); i++){
            token_name_iterator = iterator.getText();
            if("main".equals(token_name_iterator)){
                toks[i] = new String[]{String.valueOf(iterator.getLine()), String.valueOf(iterator.getCharPositionInLine()), "bold", iterator.getText()};
            }else if ("matrix".equals(token_name_iterator)){
                toks[i] = new String[]{String.valueOf(iterator.getLine()), String.valueOf(iterator.getCharPositionInLine()), "bold", iterator.getText()};
            }else if ("int".equals(token_name_iterator)){
                toks[i] = new String[]{String.valueOf(iterator.getLine()), String.valueOf(iterator.getCharPositionInLine()), "bold", iterator.getText()};
                ...
                toks[i] = new String[]{String.valueOf(iterator.getLine()), String.valueOf(iterator.getCharPositionInLine()), "blue", iterator.getText()};
            }
            iterator = (CommonToken) the_tokens.get(i);
        }
        ...
        for(Integer i = 0; i < toks.length; i++){
            if(toks[i] != null) {
                for (Integer j = 0; j < toks[i].length; j++) {
                    System.out.print(toks[i][j] + ", ");
                }
                System.out.print("\n");
            }
        }
